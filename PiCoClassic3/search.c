...
void search_lefthand(void)
{
	...
	straight(HALF_SECTION,0,SEARCH_SPEED,SEARCH_SPEED);	//まず、半区画進む
		
	while(1)
	{
		if(sen_l.is_wall == false)								//左に壁がなければ左に進む
		{
			straight(HALF_SECTION,SEARCH_SPEED,SEARCH_SPEED,0);	//半区画進んで
		...
		{
			straight(SECTION,SEARCH_SPEED,SEARCH_SPEED,SEARCH_SPEED);//一区画進む
		}
		else if(sen_r.is_wall == false)								//右に壁がなければ右に進む
		{
			straight(HALF_SECTION,SEARCH_SPEED,SEARCH_SPEED,0);		//半区画進む
...
//引数gx,gyに向かって足立法で迷路を探索する
	...
	speed = SEARCH_SPEED;
	max_speed=min_speed=SEARCH_SPEED;
	...
	while((mypos.x != gx) || (mypos.y != gy)){							//ゴールするまで繰り返す
		...
		wall_value[0][iMem] = sen_fr.value;
		wall_value[1][iMem] = sen_r.value;
		wall_value[2][iMem] = sen_l.value;
		wall_value[3][iMem] = sen_fl.value;
		...
		switch(get_nextdir(gx,gy,MASK_SEARCH,&glob_nextdir))			//次に行く方向を戻り値とする関数を呼ぶ
		{
			...
			case right:
				straight_for_search(HALF_SECTION,SEARCH_SPEED,0);		//半区画進んで
				rotate(right,1);				//右に曲がって
				straight(HALF_SECTION,0,SEARCH_SPEED,SEARCH_SPEED);		//半区画進む
				break;
